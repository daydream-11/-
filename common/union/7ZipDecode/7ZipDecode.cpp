// 7ZipDecode.cpp : Defines the entry point for the console application.
//
...
#define NORMALIZE_2 if (range < kTopValue) { range <<= 8; code = (code << 8) | (*buf++); }

#define IF_BIT_0_2(p) ttt = *(p); NORMALIZE_2; bound = (range >> kNumBitModelTotalBits) * ttt; if (code < bound)
...
#define NORMALIZE_2_CHECK if (range < kTopValue) { if (buf >= bufLimit) return DUMMY_ERROR; range <<= 8; code = (code << 8) | (*buf++); }

#define IF_BIT_0_2_CHECK(p) ttt = *(p); NORMALIZE_2_CHECK; bound = (range >> kNumBitModelTotalBits) * ttt; if (code < bound)
#define UPDATE_0_2_CHECK range = bound;
#define UPDATE_1_2_CHECK range -= bound; code -= bound;
#define GET_BIT2_CHECK(p, i, A0, A1) IF_BIT_0_2_CHECK(p) \
{ UPDATE_0_2_CHECK; i = (i + i); A0; } else \
{ UPDATE_1_2_CHECK; i = (i + i) + 1; A1; }
...
BOOL CreateDirectoryPath(LPCTSTR strDirectoryPath)
{
	INT nLen = (INT)_tcslen(strDirectoryPath);
	//TCHAR* szBuffer = new TCHAR[nLen + 1];
	TCHAR szBuffer[MAX_PATH] = { 0 };
	_tcscpy_s(szBuffer, nLen + 1, strDirectoryPath);
	bool bFlag = true;
...
void ReNameOfReRebootDelete(TCHAR * lpszExistingFileName)
{
	...
	while (TRUE)
	{
		...
		{
			//
			_stprintf_s(szBakNewFileName, _T("%s.del"), lpszExistingFileName);
		}
		else
		{
			//
			_stprintf_s(szBakNewFileName, _T("%s.del%d"), lpszExistingFileName, nCopyTimes);
		}
		//
...
int DoRunCompress(const wchar_t *lpszExtractPath, const char *lpszCompressFile, IExtractCallback* pExtractCallback)
{
	...
	UInt16 *temp = NULL;
	UInt16 nTempBufferLen = 500;
	UInt16 *tempFull = (UInt16 *)SzAlloc(NULL, nTempBufferLen);
	UInt16 nExtractPathLen = wcslen(lpszExtractPath);
	size_t tempSize = 0;
	...
	if (InFile_Open(&archiveStream.file, lpszCompressFile))
	{
		return 1;
	}
	...
	if (res == SZ_OK)
	{
		if (res == SZ_OK)		
		{
			...
			for (i = 0; i < db.db.NumFiles; i++)
			{
				...
				size_t j;
				memset(tempFull, 0, nTempBufferLen);
				wcscpy((wchar_t*)tempFull, lpszExtractPath);
				UInt16 *name = (UInt16 *)temp;

				const UInt16 *destPath = (const UInt16 *)tempFull;
				for (j = 0; name[j] != 0; j++)
				{
					if (name[j] == '/')
					{
					...
					tempFull[nExtractPathLen + j] = name[j];
				}
				CreateDirectoryPath((LPCTSTR)destPath);
				if (f->IsDir)
				{
					// callback
					if(pExtractCallback)
					{
						pExtractCallback->ExtractCallback(i + 1 , db.db.NumFiles);
					}
				...
				else if (OutFile_OpenUtf16(&outFile, destPath))
				{
					...
					_tcscpy(szTempFile, (const TCHAR*)destPath);
					ReNameOfReRebootDelete(szTempFile);
