...
//以普通方式读一字符串数据
CHAR *Ini::ReadText(CHAR *index, CHAR *name, CHAR* str, INT size, CHAR *defValue)
{
	CHAR szTmp[512] ;
	memset( szTmp, 0, 512 ) ;
	sprintf( szTmp, "[%s][%s][%s]", m_strFileName, index, name ) ;
...
//读一个整数
INT Ini::ReadInt(CHAR *index, CHAR *name, INT nDefInt)
{
	...
	INT nSectonIndex = FindIndex(index);
	if (nSectonIndex == -1)
	{
		return nDefInt;
	}

	INT nKeyIndex = FindData(nSectonIndex, name);
	if (nKeyIndex == -1)
	{
		return nDefInt;
	}
	
	CHAR *str = ReadText(nKeyIndex);
	INT nReturn = atoi(str);
	return nReturn;
}
